 ...
 */
export const escalarPorLargura = (
  valor: number,
  larguraPadrao: number = 375
): number => {
 ...
 */
export const escalarPorAltura = (
  valor: number,
  alturaPadrao: number = 667
): number => {
 ...
 */
export const obterTamanhoFonte = (
  tamanho: "xs" | "sm" | "base" | "lg" | "xl" | "2xl" | "3xl"
): number => {
  const { isSmallScreen } = useResponsividade();
 ...
 */
export const obterPaddingContainer = (): {
  horizontal: number;
  vertical: number;
} => {
  const { isSmallScreen } = useResponsividade();
