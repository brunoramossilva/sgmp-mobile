...
const ordensIniciais: OrdemServico[] = [
  {
    ...
    descricao: "Há um vazamento na torneira do banheiro do apartamento 101.",
  ...
  },
  {
    ...
    descricao: "A lâmpada do corredor do 3º andar está queimada.",
    local: "Bloco B - 3º Andar",
  ...
  },
  {
    ...
    descricao: "A porta do salão de festas não está fechando corretamente.",
    local: "Área Comum - Salão de Festas",
...
export default function TelaInicial() {
  ...
  // Função para aceitar uma OS
  const aceitarOS = (id: number) => {
    setOrdens((prev) =>
      prev.map((os) => (os.id === id ? { ...os, status: "Aceita" } : os))
    );
    setModalDetalhesVisivel(false);
  };

  // Função para recusar uma OS
  const recusarOS = (id: number) => {
    setOrdens((prev) =>
      prev.map((os) => (os.id === id ? { ...os, status: "Recusada" } : os))
    );
    setModalDetalhesVisivel(false);
  };
  ...
  // Filtrar ordens pendentes e aceitas
  ...
  return (
    <View className="flex-1 bg-slate-100">
      <ScrollView className="flex-1 px-4 pt-4">
        {/* Seção de Notificações - Ordens Pendentes */}
        <View className="mb-6">
        ...
        <View className="mb-6">
          ...
          {ordensAceitas.length === 0 ? (
            <View className="bg-white p-4 rounded-2xl border border-slate-200">
              <Text className="text-slate-500 text-center">
                Nenhuma ordem de serviço aceita no momento.
              </Text>
            </View>
          ) : (
          ...
          <View className="bg-white w-full rounded-2xl p-6 max-h-[80%]">
            ...
            {ordemSelecionada && (
              <ScrollView>
                ...
                {/* Botões de ação apenas para OS Pendentes */}
                {ordemSelecionada.status === "Pendente" && (
                  <View className="flex-row mt-4 space-x-2">
                    <TouchableOpacity
                      onPress={() => aceitarOS(ordemSelecionada.id)}
                    ...
                    <TouchableOpacity
                      onPress={() => recusarOS(ordemSelecionada.id)}
