 ...
 */
function getNavItemsByRole(papel: "MORADOR" | "FUNCIONARIO" | "SINDICO"): NavItem[] {
  switch (papel) {
    case "MORADOR":
      return [
        { id: "home", label: "Início", routeName: "InicialMorador" },
        { id: "criar-os", label: "Criar OS", routeName: "CriacaoOsMorador" },
        { id: "historico", label: "Histórico", routeName: null, disabled: true },
      ];
    ...
    case "SINDICO":
      return [
        ...
        { id: "financeiro", label: "Financeiro", routeName: null, disabled: true },
 ...
 */
function getInitialRouteByRole(papel: "MORADOR" | "FUNCIONARIO" | "SINDICO"): string {
  switch (papel) {
    case "MORADOR":
 ...
 */
const NavbarGlobal: React.FC<NavbarGlobalProps> = React.memo(({ onBackPress }) => {
  ...
  const initialRoute = usuario?.papel ? getInitialRouteByRole(usuario.papel) : "Login";
  ...
  const handleNavigation = useCallback(
    (item: NavItem) => {
      ...
      const rotasValidas = navItems.filter(i => i.routeName).map(i => i.routeName);
    ...
    <View className="absolute bottom-6 inset-x-4 bg-red-600 py-3 px-2 rounded-2xl shadow-xl">
      <View className="flex-row justify-around items-center">
        {navItems.map((item) => {
          ...
          const isFocused = item.routeName ? route.name === item.routeName : false;
          ...
          return (
            <TouchableOpacity
              ...
              accessibilityState={{ disabled: item.disabled, selected: isFocused }}
